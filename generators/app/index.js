...
module.exports = class extends BaseBlueprintGenerator {
  constructor(args, opts) {
    ...
    this.option('dto-suffix', { desc: 'Add suffix after dtos name', type: String, defaults: 'DTO' });

    // This adds support for a `--yarn` flag
    this.option('yarn', { desc: 'Use yarn instead of npm', type: Boolean, defaults: false });
  ...
  _configuring() {
    return {
      ...
      composeClient() {
        ...
        this.composeWith(require.resolve('../client'), { ...options, configOptions, debug: this.isDebugEnabled });
      },
  ...
  _default() {
    return {
      ...
      insight() {
        const yorc = {
          ..._.omit(this.configOptions, ['jhiPrefix', 'baseName', 'jwtSecretKey', 'packageName', 'packagefolder', 'rememberMeKey']),
        };
                  ...
                  if (code === 0) {
                    ...
                    if (this.blueprints && this.blueprints.length > 0) {
                      const bpInfo = this.blueprints
                        .map(bp => `${bp.name.replace('generator-finscale-hipsters-', '')}-${bp.version}`)
                        .join(', ');
      ...
      afterRunHook() {
        try {
          ...
          if (modules.length > 0) {
            ...
            this.callHooks('app', 'post', { appConfig: this.configOptions, force: this.options.force });
          }
        } catch (err) {
