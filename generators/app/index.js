...
module.exports = class extends BaseBlueprintGenerator {
  constructor(args, opts) {
    ...
    this.option('from-cli', {
      desc : 'Indicates the command is run from Finscale-Hipsters CLI',
      type : Boolean,
      defaults : false
    });
    ...
    this.option('skip-check-length-of-identifier', {
      desc :
          'Skip check the length of the identifier, only for recent Oracle databases that support 30+ characters metadata',
      type : Boolean,
      defaults : false
    });
    ...
    this.option(
        'dto-suffix',
        {desc : 'Add suffix after dtos name', type : String, defaults : 'DTO'});

    // This adds support for a `--yarn` flag
    this.option(
        'yarn',
        {desc : 'Use yarn instead of npm', type : Boolean, defaults : false});

    // This adds support for a `--auth` flag
    this.option('auth', {
      desc :
          'Provide authentication type for the application when skipping server side generation',
      type : String
    });
    ...
    this.option('db', {
      desc :
          'Provide DB name for the application when skipping server side generation',
      type : String
    });
    ...
    this.option('uaa-base-name', {
      desc :
          'Provide the name of UAA server, when using --auth uaa and skipping server side generation',
      type : String
    });
    ...
    this.option('build', {
      desc :
          'Provide build tool for the application when skipping server side generation',
      type : String
    });
    ...
    this.option('websocket', {
      desc :
          'Provide websocket option for the application when skipping server side generation',
      type : String
    });
    ...
    this.option('search-engine', {
      desc :
          'Provide search engine for the application when skipping server side generation',
      type : String
    });
    ...
    this.option('blueprint', {
      desc :
          'DEPRECATED: Specify a generator blueprint to use for the sub generators',
      type : String
    });
    // This adds support for a `--blueprints` flag which can be used to specify
    // one or more blueprints to use for generation
    ...
    // This adds support for a `--creation-timestamp` flag which can be used
    ...
    this.skipClient = this.configOptions.skipClient =
        this.options['skip-client'] || this.config.get('skipClient');
    this.skipServer = this.configOptions.skipServer =
        this.options['skip-server'] || this.config.get('skipServer');
    this.skipUserManagement = this.configOptions.skipUserManagement =
        this.options['skip-user-management'] ||
        this.config.get('skipUserManagement');
    this.skipCheckLengthOfIdentifier =
        this.configOptions.skipCheckLengthOfIdentifier =
            this.options['skip-check-length-of-identifier'] ||
            this.config.get('skipCheckLengthOfIdentifier');
    this.skipFakeData = this.configOptions.skipFakeData =
        this.options['skip-fake-data'];
    this.jhiPrefix = this.configOptions.jhiPrefix =
        _.camelCase(this.config.get('jhiPrefix') || this.options['jhi-prefix']);
    this.uaaBaseName = this.configOptions.uaaBaseName =
        this.options['uaa-base-name'] || this.config.get('uaaBaseName');

    this.entitySuffix = this.configOptions.entitySuffix =
        _.isNil(this.config.get('entitySuffix'))
            ? this.options['entity-suffix']
            : this.config.get('entitySuffix');

    this.dtoSuffix = this.configOptions.dtoSuffix =
        _.isNil(this.config.get('dtoSuffix')) ? this.options['dto-suffix']
    ...
    if (blueprint) {
      this.warning(
          '--blueprint option is deprecated. Please use --blueprints instead');
    ...
    this.isDebugEnabled = this.configOptions.isDebugEnabled =
        this.options.debug;
    this.experimental = this.configOptions.experimental =
  ...
  _initializing() {
    return {
      ...
      displayLogo() { this.printJHipsterLogo(); },
      ...
      validateJava() { this.checkJava(); },

      validateNode() { this.checkNode(); },

      validateGit() { this.checkGit(); },

      validateYarn() { this.checkYarn(); },
      ...
      validate() {
        if (this.skipServer && this.skipClient) {
          this.error(
              `You can not pass both ${chalk.yellow('--skip-client')} and ${
                  chalk.yellow('--skip-server')} together`);
        }
      },
      ...
      setupconsts() {
        ...
        if (this.blueprints && this.blueprints.length > 0) {
          this.blueprints.forEach(blueprint => {
            blueprint.version = this.findBlueprintVersion(blueprint.name) ||
                                blueprint.version || 'latest';
          });

          // Remove potential previous value to avoid duplicates
          this.otherModules = this.otherModules.filter(
              module => this.blueprints.findIndex(bp => bp.name ===
                                                        module.name) === -1);
        ...
        const configFound =
            this.baseName !== undefined && this.applicationType !== undefined;
        if (configFound) {
        ...
        // Set embeddableLaunchScript to true if not defined, for backward
        // compatibility See
        // https://github.com/jhipster/generator-jhipster/issues/10255
    ...
    return {
      setup() {
        ...
        if (this.skipServer) {
          ...
          this.configOptions.databaseType =
      ...
      composeClient() {
        ...
        this.composeWith(
            ...
            {...options, configOptions, debug : this.isDebugEnabled});
      },
  ...
  _default() {
    return {
      ...
      composeLanguages() {
        if (this.skipI18n || this.configOptions.skipComposeLanguages)
          return;
      ...
      saveConfig() {
        ...
        this.blueprintVersion &&
            (config.blueprintVersion = this.blueprintVersion);
      ...
      insight() {
        const yorc = {..._.omit(this.configOptions, [
          'jhiPrefix', 'baseName', 'jwtSecretKey', 'packageName',
          'packagefolder', 'rememberMeKey'
        ])};
      ...
      initGitRepo() {
        if (!this.options['skip-git']) {
          if (this.gitInstalled || this.isGitInstalled()) {
            const gitDir =
                this.gitExec('rev-parse --is-inside-work-tree', {trace : false})
                    .stdout;
            ...
            this.warning(
                'Git repository could not be initialized, as Git is not installed on your system');
          }
        }
      }
    };
      ...
      gitCommit() {
        if (!this.options['skip-git'] && this.isGitInstalled()) {
          if (this.gitInitialized) {
            ...
            this.gitExec(
                'log --oneline -n 1 -- .', {trace : false}, (code, commits) => {
                  if (code !== 0 || !commits || !commits.trim()) {
                    // if no files in Git from current folder then we assume
                    // that this is initial application generation
                    this.gitExec('add .', {trace : false}, code => {
                      if (code === 0) {
                        let commitMsg = `Initial version of ${
                            this.baseName} generated by JHipster-${
                        ...
                        if (this.blueprints && this.blueprints.length > 0) {
                          const bpInfo =
                              this.blueprints
                                  .map(bp => `${
                                           bp.name.replace(
                                               'generator-finscale-hipsters-',
                                               '')}-${bp.version}`)
                                  .join(', ');
                          commitMsg += ` with blueprints: ${bpInfo}`;
                        }
                        this.gitExec(
                            `commit -m "${commitMsg}" -- .`, {trace : false},
                            code => {
                              if (code === 0) {
                                this.log(chalk.green.bold(
                                    `Application successfully committed to Git from ${
                                        process.cwd()}.`));
                              } else {
                                this.log(chalk.red.bold(
                                    `Application commit to Git failed from ${
                                        process
                                            .cwd()}. Try to commit manually.`));
                              }
                              done();
                            });
                      } else {
                        this.warning(
                            `The generated application could not be committed to Git, because ${
                                chalk.bold('git add')} command failed.`);
                        done();
                      }
                    });
                  } else {
                    // if found files in Git from current folder then we assume
                    // that this is application regeneration if there are
                    // changes in current folder then inform user about manual
                    // commit needed
                    this.gitExec(
                        'diff --name-only .', {trace : false},
                        (code, diffs) => {
                          if (code === 0 && diffs && diffs.trim()) {
                            this.log(`Found commits in Git from ${
                                process
                                    .cwd()}. So we assume this is application regeneration. Therefore automatic Git commit is not done. You can do Git commit manually.`);
                          }
                          done();
                        });
                  }
      ...
      afterRunHook() {
        try {
          ...
          if (modules.length > 0) {
            ...
            this.callHooks(
                'app', 'post',
                {appConfig : this.configOptions, force : this.options.force});
          }
        } catch (err) {
          this.log(`\n${
              chalk.bold.red(
                  'Running post run module hooks failed. No modification done to the generated app.')}`);
          this.debug('Error:', err);
        }
        this.log(chalk.green(
            `\nIf you find JHipster useful consider sponsoring the project ${
                chalk.yellow('https://www.jhipster.tech/sponsors/')}`));
      }
    };
  }
